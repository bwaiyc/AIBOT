...
class SafeConfigDict(dict):
    """Dict with soft defaults for missing config keys."""
    ...
    def __missing__(self, key):
        ...
        if key_str in bool_like_keys or key_str.startswith('is_') or key_str.startswith('mask_') or 'mask' in key_str:
            value = False
        elif key_str.endswith('_ip'):
            value = '127.0.0.1'
        elif key_str.endswith('_port'):
            value = 0
        elif key_str.endswith('_fps'):
            value = 60
        elif key_str.endswith('_uuid'):
            value = ''
        elif key_str.endswith('_model'):
            value = 'model.onnx'
        elif key_str.endswith('_resolution') or key_str.endswith('_crop_size'):
            value = ''
        elif key_str in ('move_method', 'controller_type'):
            value = 'send_input'
        else:
            value = 0
        self[key] = value
...
def _to_safe_config(value):
    if isinstance(value, dict) and not isinstance(value, SafeConfigDict):
        safe = SafeConfigDict()
        for k, v in value.items():
            safe[k] = _to_safe_config(v)
        return safe
    if isinstance(value, list):
        return [_to_safe_config(v) for v in value]
    return value
...
class Valorant:
    ...
    # ================== 修改3：build_config 从本地 cfg.json 加载 ==================
    def build_config(self):
        """
        从本地 cfg.json 加载配置，不再依赖远程服务器。
        """
        if hasattr(self, 'config') and self.config:
            ...
            if not os.path.exists(cfg_file):
                raise RuntimeError(f'配置文件 {cfg_file} 不存在，请将有效的 cfg.json 放在程序目录下。')
            with open(cfg_file, 'r', encoding='utf-8') as f:
                config = json.load(f)
        config = _to_safe_config(config)
        config.setdefault('group', 'default')
        config.setdefault('groups', SafeConfigDict())
        config.setdefault('games', SafeConfigDict(
            {'default_game': SafeConfigDict({'default_gun': [SafeConfigDict({'number': 999999, 'offset': [0, 0]})]})}))
        config.setdefault('picked_game', next(iter(config['games'].keys())) if config['games'] else 'default_game')
        if config['group'] not in config['groups']:
            config['groups'][config['group']] = SafeConfigDict()
        current_group_cfg = config['groups'][config['group']]
        current_group_cfg.setdefault('infer_model', 'model.onnx')
        current_group_cfg.setdefault('is_v8', True)
        current_group_cfg.setdefault('is_trt', False)
        current_group_cfg.setdefault('right_down', False)
        current_group_cfg.setdefault('long_press_duration', 0.2)
        current_group_cfg.setdefault('aim_keys', SafeConfigDict())
        if not current_group_cfg['aim_keys']:
            current_group_cfg['aim_keys']['mouse_right'] = SafeConfigDict(
                {'status': True, 'aim_bot': True, 'trigger_bot': False, 'auto_y': False, 'aim_bot_position': 0.5,
                 'aim_bot_position2': 0.5, 'aim_bot_scope': 120.0, 'target_reference_class': 0,
                 'class_aim_positions': SafeConfigDict({'0': 0.5}), 'trigger': SafeConfigDict(
                    {'status': False, 'continuous': False, 'recoil': False, 'start_delay': 0, 'press_delay': 1,
                     'end_delay': 0, 'random_delay': 0, 'x_trigger_scope': 1.0, 'y_trigger_scope': 1.0,
                     'x_trigger_offset': 0.0, 'y_trigger_offset': 0.0})})
        for _group_cfg in config['groups'].values():
            _group_cfg.setdefault('aim_keys', SafeConfigDict())
            for _key_cfg in _group_cfg['aim_keys'].values():
                _key_cfg.setdefault('classes', [])
                _key_cfg.setdefault('status', True)
                _key_cfg.setdefault('aim_bot', True)
                _key_cfg.setdefault('trigger_bot', False)
                _key_cfg.setdefault('auto_y', False)
                _key_cfg.setdefault('aim_bot_scope', 120.0)
                _key_cfg.setdefault('aim_bot_position', 0.5)
                _key_cfg.setdefault('aim_bot_position2', 0.5)
                _key_cfg.setdefault('target_reference_class', 0)
                _key_cfg.setdefault('class_aim_positions', SafeConfigDict({'0': SafeConfigDict(
                    {'aim_bot_position': 0.5, 'aim_bot_position2': 0.5, 'confidence_threshold': 0.5, 'iou_t': 1.0})}))
                if isinstance(_key_cfg.get('class_aim_positions'), dict):
                    _fixed_class_cfg = SafeConfigDict()
                    for _class_id, _class_cfg in _key_cfg['class_aim_positions'].items():
                        if isinstance(_class_cfg, dict):
                            _fixed_class_cfg[str(_class_id)] = _to_safe_config(_class_cfg)
                        else:
                            _fixed_class_cfg[str(_class_id)] = SafeConfigDict(
                                {'aim_bot_position': float(_class_cfg), 'aim_bot_position2': float(_class_cfg),
                                 'confidence_threshold': 0.5, 'iou_t': 1.0})
                    _key_cfg['class_aim_positions'] = _fixed_class_cfg
                _key_cfg.setdefault('trigger', SafeConfigDict(
                    {'status': False, 'continuous': False, 'recoil': False, 'start_delay': 0, 'press_delay': 1,
                     'end_delay': 0, 'random_delay': 0, 'x_trigger_scope': 1.0, 'y_trigger_scope': 1.0,
                     'x_trigger_offset': 0.0, 'y_trigger_offset': 0.0}))
        ...
        # 初始化背闪配置并添加 flash_class
        if 'auto_flashbang' not in config:
            config['auto_flashbang'] = {'enabled': False, 'delay_ms': 150, 'turn_angle': 90,
                                        'sensitivity_multiplier': 2.5, 'return_delay': 80, 'min_confidence': 0.3,
                                        'min_size': 5, 'use_curve': True, 'curve_speed': 8.0, 'curve_knots': 3,
                                        'flash_class': 4}
        else:
            if 'flash_class' not in config['auto_flashbang']:
                config['auto_flashbang']['flash_class'] = 4
    ...
    # ================== 删除所有 Buff 验证相关函数 ==================
    # 包括 start_verify_init, error_exit, verify 等方法均删除。
    # 但为了兼容性，可以保留空方法或直接注释调用。
    # ==============================================================

    # ================== 修改4：start 方法跳过验证 ==================
    def start(self):
        # 不再初始化 Buff_Single，也不进行验证
        self.verified = True
        self.gui()
    # ==============================================================
    ...
    # ================== 修改7：on_start_button_click 移除 verify 调用 ==================
        ...
        if current_label == '启动':
            ...
            if self.config['groups'][self.group].get('is_trt', False) and TENSORRT_AVAILABLE:
                ...
                if not os.path.exists(engine_path):
                    ...
                    # 安全获取 decrypted_model_data
                    if current_model.endswith('.ZTX') and getattr(self, 'decrypted_model_data', None) is not None:
                        if ensure_engine_from_memory is not None:
                            final_engine_path = ensure_engine_from_memory(self.decrypted_model_data, engine_path,
                                                                          target_hw=None)
            ...
            self.stop_listen()
            dpg.configure_item(sender, label='启动')
    # ================== 以下为原 core.py 中其余全部方法 ==================
    # 注意：请直接复制原 core.py 中从 down_func 开始到文件末尾的所有方法，
    # 但要删除与 Buff 验证相关的函数定义（如 start_verify_init, error_exit, verify），
    # 并确保 self.verified 已被直接设为 True，不再调用验证。
    ...
    # 注意：以下删除了 start_verify_init, error_exit, verify 三个与验证相关的方法
    ...
    def aim_bot_func(self, uTimerID, uMsg, dwUser, dw1, dw2):
        if self.aim_key_status:
            ...
            if len(aim_targets):
                ...
                # --- 修改部分：动态获取用户设置的闪光类别 ---
                target_flash_class = self.config['auto_flashbang'].get('flash_class', 4)
                ...
                for i in range(len(aim_targets)):
                    ...
                    # --- 过滤逻辑：如果识别到的是闪光目标，则跳过，不纳入自瞄锁定范围 ---
                    if self.config['auto_flashbang']['enabled'] and class_id == target_flash_class:
                        continue
                    ...
                    target_obj = {'pos': (self.identify_rect_left + result_center_x,
                                          self.identify_rect_top + (result_center_y - height / 2) + max(
                                              height * aim_position, self.pressed_key_config['min_position_offset'])),
                                  'size': final_size_score, 'absolute_size': absolute_area,
                                  'relative_size': relative_size, 'class_id': class_id, 'aim_position': aim_position,
                ...
                if aim_bot_scope > 0 and len(target_objects) > 0:
                    ...
                    target_objects = [t for t in target_objects if
                                      (t['pos'][0] - cx) * (t['pos'][0] - cx) + (t['pos'][1] - cy) * (
                                                  t['pos'][1] - cy) <= max_dist_sq]
                if self.config['small_target_enhancement']['enabled'] and self.config['small_target_enhancement'][
                    'smooth_enabled']:
                ...
                if nearest is not None:
                    ...
                    if self.aim_key_status:
                        ...
                        if current_key in self.aim_keys_dist and self.aim_keys_dist[current_key].get('auto_y',
                                                                                                     False) and self.left_pressed_long:
                            relative_move_y = 0
        ...
        while self.running:
            ...
            if len(boxes) > 0:
                ...
                if selected_classes_set:
                    ...
                    classes = []
                    class_ids = []
                    # --- 彻底移除旧版ZTX模型验证，使用动态闪光类别 ---
                    if self.config['auto_flashbang']['enabled']:
                        target_flash_class = self.config['auto_flashbang'].get('flash_class', 4)
                        if len(boxes) > 0 and len(class_ids) > 0:
                            self.detect_and_handle_flashbang(boxes, class_ids, input_shape_weight, input_shape_height,
                                                             scores)
                        elif len(class_ids) > 0 and target_flash_class in class_ids:
                            # 识别到了目标，但可能因为太小或置信度太低被过滤掉了
                            print(f'检测到闪光类别 {target_flash_class}，但因置信度/尺寸过小被过滤')
            if len(boxes) > 0:
                if self.aim_key_status:
                    try:
    ...
    def get_gradient_color(self, base_color, step):
        """ 根据基色生成颜色渐变 """
        r, g, b, a = base_color
        factor = 1 + step
    ...
    def gui(self):
        ...
        def switch_tab(sender, app_data, user_data):
            all_tabs = ['system_settings', 'driver_settings', 'bypass_settings', 'strafe_settings', 'config_settings']
            for tab_tag in all_tabs:
                if dpg.does_item_exist(tab_tag):
                    dpg.configure_item(tab_tag, show=(tab_tag == user_data))

        with dpg.window(label=title, no_title_bar=True, no_resize=True, no_move=True, width=self.gui_window_width,
                        height=self.gui_window_height) as self.window_tag:
            ...
            with dpg.group(horizontal=True):
                ...
                system_btn = dpg.add_button(label='s', width=button_size, height=button_size, callback=switch_tab,
                                            user_data='system_settings')
                driver_btn = dpg.add_button(label='v', width=button_size, height=button_size, callback=switch_tab,
                                            user_data='driver_settings')
                bypass_btn = dpg.add_button(label='o', width=button_size, height=button_size, callback=switch_tab,
                                            user_data='bypass_settings')
                strafe_btn = dpg.add_button(label='W', width=button_size, height=button_size, callback=switch_tab,
                                            user_data='strafe_settings')
                config_btn = dpg.add_button(label='u', width=button_size, height=button_size, callback=switch_tab,
                                            user_data='config_settings')
                for btn in [system_btn, driver_btn, bypass_btn, strafe_btn, config_btn]:
                    dpg.bind_item_font(btn, custom_font)

            # --- 全宽内容区域 ---
            content_height = self.gui_window_height - 120 - button_size
            with dpg.child_window(width=self.gui_window_width, height=content_height) as tab_bar_container:
                dpg.bind_item_theme(tab_bar_container, tab_bar_theme)
                dpg.bind_theme(skeet_theme)
                with dpg.group(tag='tab_pages_container'):
                    # ================== 1. 系统设置页 ==================
                    with dpg.group(tag='system_settings', show=True):
                        with dpg.group(horizontal=True):
                            self.dpi_scale_slider = dpg.add_slider_float(label='GUI DPI缩放',
                                                                         default_value=self.dpi_scale, min_value=0.5,
                        ...
                        dpg.add_text(
                            f'调整GUI界面的缩放大小 (当前系统检测: {self.get_system_dpi_scale():.2f}, 重启后生效)')
                        dpg.add_separator()
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='推理窗口', default_value=self.config['infer_debug'],
                                             callback=self.on_infer_debug_change)
                            dpg.add_checkbox(label='打印帧率', default_value=self.config['print_fps'],
                                             callback=self.on_print_fps_change)
                            dpg.add_checkbox(label='显示运动速度', default_value=self.config['show_motion_speed'],
                                             callback=self.on_show_motion_speed_change)
                            dpg.add_checkbox(label='显示曲线', default_value=self.config['is_show_curve'],
                                             callback=self.on_is_show_curve_change)
                            dpg.add_checkbox(label='显示推理时间',
                                             default_value=self.config.get('show_infer_time', True),
                                             callback=self.on_show_infer_time_change)
                            dpg.add_checkbox(label='截图分离(多线程)',
                                             default_value=self.config.get('enable_parallel_processing', True),
                                             callback=self.on_enable_parallel_processing_change)
                        dpg.add_separator()
                        dpg.add_text('小目标识别增强设置', color=(100, 200, 255))
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='启用小目标识别增强', tag='small_target_enabled_checkbox',
                                             default_value=self.config['small_target_enhancement']['enabled'],
                                             callback=self.on_small_target_enabled_change)
                            dpg.add_checkbox(label='启用小目标平滑', tag='small_target_smooth_checkbox',
                                             default_value=self.config['small_target_enhancement']['smooth_enabled'],
                                             callback=self.on_small_target_smooth_change)
                            dpg.add_checkbox(label='自适应NMS', tag='small_target_nms_checkbox',
                                             default_value=self.config['small_target_enhancement']['adaptive_nms'],
                                             callback=self.on_small_target_nms_change)
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='小目标增强倍数', tag='small_target_boost_input',
                                                default_value=self.config['small_target_enhancement']['boost_factor'],
                                                min_value=1.0, max_value=5.0, step=0.1,
                                                callback=self.on_small_target_boost_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_int(label='平滑历史帧数', tag='small_target_frames_input',
                                              default_value=self.config['small_target_enhancement']['smooth_frames'],
                                              min_value=2, max_value=10, callback=self.on_small_target_frames_change,
                                              width=self.scaled_width_medium)
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='小目标阈值', tag='small_target_threshold_input',
                                                default_value=self.config['small_target_enhancement']['threshold'],
                                                min_value=0.001, max_value=0.1, step=0.001, format='%.3f',
                                                callback=self.on_small_target_threshold_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_float(label='中等目标阈值', tag='medium_target_threshold_input',
                                                default_value=self.config['small_target_enhancement'][
                                                    'medium_threshold'], min_value=0.01, max_value=0.2, step=0.01,
                                                format='%.3f', callback=self.on_medium_target_threshold_change,
                                                width=self.scaled_width_normal)
                        dpg.add_text('说明：小目标识别增强可以提高对远距离或小尺寸目标的检测稳定性',
                                     color=(150, 150, 150), wrap=self.scaled_width_xlarge)
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='强制提速模式', default_value=self.config.get('turbo_mode', True),
                                             callback=self.on_turbo_mode_change)
                            dpg.add_checkbox(label='跳过帧处理',
                                             default_value=self.config.get('skip_frame_processing', True),
                                             callback=self.on_skip_frame_processing_change)
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='压枪调试', default_value=self.config['is_show_down'],
                                             callback=self.on_is_show_down_change)
                            dpg.add_checkbox(label='类别优先级调试',
                                             default_value=self.config.get('is_show_priority_debug', False),
                                             callback=self.on_is_show_priority_debug_change)
                            dpg.add_checkbox(label='显示瞄准范围', default_value=self.config.get('show_fov', True),
                                             callback=self.on_show_fov_change)
                        dpg.add_checkbox(label='OBS', default_value=self.config['is_obs'],
                                         callback=self.on_is_obs_change)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='OBS IP', default_value=self.config['obs_ip'],
                                               callback=self.on_obs_ip_change, width=self.scaled_width_normal)
                            dpg.add_input_int(label='OBS 端口', default_value=self.config['obs_port'],
                                              callback=self.on_obs_port_change, width=self.scaled_width_normal)
                            dpg.add_input_int(label='OBS 帧率', default_value=self.config['obs_fps'],
                                              callback=self.on_obs_fps_change, width=self.scaled_width_normal)
                        dpg.add_checkbox(label='采集卡', default_value=self.config['is_cjk'],
                                         callback=self.on_is_cjk_change)
                        with dpg.group(horizontal=True):
                            dpg.add_input_int(label='采集卡设备', default_value=self.config['cjk_device_id'],
                                              callback=self.on_cjk_device_id_change, width=self.scaled_width_normal)
                            dpg.add_input_int(label='采集卡FPS', default_value=self.config['cjk_fps'],
                                              callback=self.on_cjk_fps_change, width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='采集卡分辨率', default_value=self.config['cjk_resolution'],
                                               callback=self.on_cjk_resolution_change, width=self.scaled_width_medium)
                            dpg.add_input_text(label='采集卡裁剪尺寸', default_value=self.config['cjk_crop_size'],
                                               callback=self.on_cjk_crop_size_change, width=self.scaled_width_medium)
                        dpg.add_input_text(label='视频编码格式',
                                           default_value=self.config.get('cjk_fourcc_format', 'NV12'),
                                           callback=self.on_cjk_fourcc_format_change, width=self.scaled_width_medium,
                                           hint='如: NV12, MJPG, YUYV')
                        dpg.add_text('自瞄权重')
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='距离权重', default_value=self.config['distance_scoring_weight'],
                                                min_value=0.0, step=0.05,
                                                callback=self.on_distance_scoring_weight_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_float(label='中心权重', default_value=self.config['center_scoring_weight'],
                                                min_value=0.0, step=0.05, callback=self.on_center_scoring_weight_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_float(label='大小权重', default_value=self.config['size_scoring_weight'],
                                                min_value=0.0, step=0.05, callback=self.on_size_scoring_weight_change,
                                                width=self.scaled_width_normal)

                        dpg.add_separator()
                        dpg.add_text('自动背闪设置')
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='启用自动背闪', tag='auto_flashbang_enabled_checkbox',
                                             default_value=self.config['auto_flashbang']['enabled'],
                                             callback=self.on_auto_flashbang_enabled_change)
                            dpg.add_input_int(label='闪光类别', tag='auto_flashbang_class_input',
                                              default_value=self.config['auto_flashbang'].get('flash_class', 4),
                                              min_value=0, max_value=20, callback=self.on_flash_class_change,
                                              width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_int(label='背闪延迟(毫秒)', tag='auto_flashbang_delay_input',
                                              default_value=self.config['auto_flashbang']['delay_ms'], min_value=50,
                                              max_value=2000, callback=self.on_auto_flashbang_delay_change,
                                              width=self.scaled_width_normal)
                            dpg.add_input_int(label='转向角度', tag='auto_flashbang_angle_input',
                                              default_value=self.config['auto_flashbang']['turn_angle'], min_value=45,
                                              max_value=180, callback=self.on_auto_flashbang_angle_change,
                                              width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='灵敏度倍数', tag='auto_flashbang_sensitivity_input',
                                                default_value=self.config['auto_flashbang']['sensitivity_multiplier'],
                                                min_value=0.5, max_value=9999, step=0.1,
                                                callback=self.on_auto_flashbang_sensitivity_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_int(label='回转延迟(毫秒)', tag='auto_flashbang_return_delay_input',
                                              default_value=self.config['auto_flashbang']['return_delay'],
                                              min_value=100, max_value=2000,
                                              callback=self.on_auto_flashbang_return_delay_change,
                                              width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='使用曲线移动', tag='auto_flashbang_curve_checkbox',
                                             default_value=self.config['auto_flashbang']['use_curve'],
                                             callback=self.on_auto_flashbang_curve_change)
                            dpg.add_input_float(label='曲线速度', tag='auto_flashbang_curve_speed_input',
                                                default_value=self.config['auto_flashbang']['curve_speed'],
                                                min_value=0.1, max_value=2.0, step=0.1,
                                                callback=self.on_auto_flashbang_curve_speed_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_int(label='控制点数量', tag='auto_flashbang_curve_knots_input',
                                              default_value=self.config['auto_flashbang']['curve_knots'], min_value=5,
                                              max_value=50, callback=self.on_auto_flashbang_curve_knots_change,
                                              width=self.scaled_width_normal)
                        dpg.add_text('过滤条件（调整这些参数可以改变触发灵敏度）')
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='最小置信度', tag='auto_flashbang_min_confidence_input',
                                                default_value=self.config['auto_flashbang']['min_confidence'],
                                                min_value=0.1, max_value=0.9, step=0.05, format='%.2f',
                                                callback=self.on_auto_flashbang_min_confidence_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_int(label='最小尺寸(像素)', tag='auto_flashbang_min_size_input',
                                              default_value=self.config['auto_flashbang']['min_size'], min_value=1,
                                              max_value=50, callback=self.on_auto_flashbang_min_size_change,
                                              width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_button(label='测试左转', tag='auto_flashbang_test_left_button',
                                           callback=self.on_test_flashbang_left)
                            dpg.add_button(label='测试右转', tag='auto_flashbang_test_right_button',
                                           callback=self.on_test_flashbang_right)
                            dpg.add_button(label='调试信息', tag='auto_flashbang_debug_button',
                                           callback=self.on_flashbang_debug_info)

                    # ================== 2. 驱动设置页 ==================
                    with dpg.group(tag='driver_settings', show=False):
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='移动曲线', default_value=self.config['is_curve'],
                                             callback=self.on_is_curve_change)
                            dpg.add_checkbox(label='补偿曲线', default_value=self.config['is_curve_uniform'],
                                             callback=self.on_is_curve_uniform_change)
                            dpg.add_input_int(label='横向边界', default_value=self.config['offset_boundary_x'],
                                              callback=self.on_offset_boundary_x_change, width=self.scaled_width_normal)
                            dpg.add_input_int(label='纵向边界', default_value=self.config['offset_boundary_y'],
                                              callback=self.on_offset_boundary_y_change, width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_int(label='控制点数量', default_value=self.config['knots_count'],
                                              callback=self.on_knots_count_change, width=self.scaled_width_normal)
                            dpg.add_input_float(label='扰动的平均值', default_value=self.config['distortion_mean'],
                                                callback=self.on_distortion_mean_change, width=self.scaled_width_normal)
                            dpg.add_input_float(label='扰动的标准差', default_value=self.config['distortion_st_dev'],
                                                callback=self.on_distortion_st_dev_change,
                                                width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_float(label='扰动的频率', default_value=self.config['distortion_frequency'],
                                                callback=self.on_distortion_frequency_change,
                                                width=self.scaled_width_normal)
                            dpg.add_input_int(label='路径点总数', default_value=self.config['target_points'],
                                              callback=self.on_target_points_change, width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='KM Box VID', default_value=self.config['km_box_vid'],
                                               callback=self.on_km_box_vid_change, width=self.scaled_width_small)
                            dpg.add_input_text(label='KM Box PID', default_value=self.config['km_box_pid'],
                                               callback=self.on_km_box_pid_change, width=self.scaled_width_small)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='KM Net IP', default_value=self.config['km_net_ip'],
                                               callback=self.on_km_net_ip_change, width=self.scaled_width_large)
                            dpg.add_input_text(label='KM Net Port', default_value=str(self.config['km_net_port']),
                                               callback=self.on_km_net_port_change, width=self.scaled_width_small)
                            dpg.add_input_text(label='KM Net UUID', default_value=self.config['km_net_uuid'],
                                               callback=self.on_km_net_uuid_change, width=self.scaled_width_medium)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='DHZ IP', default_value=self.config['dhz_ip'],
                                               callback=self.on_dhz_ip_change, width=self.scaled_width_large)
                            dpg.add_input_int(label='DHZ Port', default_value=self.config['dhz_port'],
                                              callback=self.on_dhz_port_change, width=self.scaled_width_normal)
                            dpg.add_input_int(label='DHZ RANDOM', default_value=self.config['dhz_random'],
                                              callback=self.on_dhz_random_change, width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='CatBox IP', default_value=self.config['catbox_ip'],
                                               callback=self.on_catbox_ip_change, width=self.scaled_width_large)
                            dpg.add_input_int(label='CatBox Port', default_value=self.config['catbox_port'],
                                              callback=self.on_catbox_port_change, width=self.scaled_width_normal)
                            dpg.add_input_text(label='CatBox UUID', default_value=self.config['catbox_uuid'],
                                               callback=self.on_catbox_uuid_change, width=self.scaled_width_medium)
                        with dpg.group(horizontal=True):
                            dpg.add_input_text(label='COM', default_value=self.config['km_com'],
                                               callback=self.on_km_com_change, width=self.scaled_width_normal)
                        with dpg.group(horizontal=True):
                            dpg.add_combo(label='移动模式',
                                          items=['send_input', 'dhz', 'km_net', 'pnmh', 'km_box_a', 'logitech', 'makcu',
                                                 'catbox'], default_value=self.config['move_method'],
                                          callback=self.on_move_method_change, width=self.scaled_width_large)

                    # ================== 3. 屏蔽设置页 ==================
                    with dpg.group(tag='bypass_settings', show=False):
                        with dpg.group(horizontal=True):
                            self.mask_left_checkbox = dpg.add_checkbox(label='屏蔽左键',
                                                                       default_value=self.config['mask_left'],
                                                                       callback=self.on_mask_left_change)
                            self.mask_right_checkbox = dpg.add_checkbox(label='屏蔽右键',
                                                                        default_value=self.config['mask_right'],
                                                                        callback=self.on_mask_right_change)
                            self.mask_middle_checkbox = dpg.add_checkbox(label='屏蔽中键',
                                                                         default_value=self.config['mask_middle'],
                                                                         callback=self.on_mask_middle_change)
                            self.mask_side1_checkbox = dpg.add_checkbox(label='屏蔽侧键1',
                                                                        default_value=self.config['mask_side1'],
                                                                        callback=self.on_mask_side1_change)
                            self.mask_side2_checkbox = dpg.add_checkbox(label='屏蔽侧键2',
                                                                        default_value=self.config['mask_side2'],
                                                                        callback=self.on_mask_side2_change)
                        with dpg.group(horizontal=True):
                            self.mask_x_checkbox = dpg.add_checkbox(label='屏蔽X轴',
                                                                    default_value=self.config['mask_x'],
                                                                    callback=self.on_mask_x_change)
                            self.mask_y_checkbox = dpg.add_checkbox(label='屏蔽Y轴',
                                                                    default_value=self.config['mask_y'],
                                                                    callback=self.on_mask_y_change)
                            self.aim_mask_x_checkbox = dpg.add_checkbox(label='瞄准时屏蔽X轴',
                                                                        default_value=self.config.get('aim_mask_x',
                                                                                                      False),
                                                                        callback=self.on_aim_mask_x_change)
                            self.aim_mask_y_checkbox = dpg.add_checkbox(label='瞄准时屏蔽Y轴',
                                                                        default_value=self.config.get('aim_mask_y',
                                                                                                      False),
                                                                        callback=self.on_aim_mask_y_change)
                            self.mask_wheel_checkbox = dpg.add_checkbox(label='屏蔽滚轮',
                                                                        default_value=self.config['mask_wheel'],
                                                                        callback=self.on_mask_wheel_change)

                    # ================== 4. 压枪设置页 ==================
                    with dpg.group(tag='strafe_settings', show=False):
                        self.right_down_checkbox = dpg.add_checkbox(label='检测右键',
                                                                    callback=self.on_right_down_change)
                        with dpg.group(horizontal=True):
                            with dpg.group() as self.dpg_games_tag:
                                self.render_games_combo()
                            dpg.add_button(label='删除游戏', callback=self.on_delete_game_click,
                                           width=self.scaled_width_60)
                            dpg.add_input_text(label='游戏名', callback=self.on_game_name_change,
                                               width=self.scaled_width_60)
                            dpg.add_button(label='添加游戏', callback=self.on_add_game_click,
                                           width=self.scaled_width_60)
                        with dpg.group(horizontal=True):
                            with dpg.group() as self.dpg_guns_tag:
                                self.render_guns_combo()
                            dpg.add_button(label='删除枪械', callback=self.on_delete_gun_click,
                                           width=self.scaled_width_60)
                            dpg.add_input_text(label='枪械名', callback=self.on_gun_name_change,
                                               width=self.scaled_width_60)
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            try:
                                ...
                                self.config['games'][self.picked_game][self.picked_gun][int(self.picked_stage)][
                                    'number']
                                x = self.config['games'][self.picked_game][self.picked_gun][int(self.picked_stage)][
                                    'offset'][0]
                                y = self.config['games'][self.picked_game][self.picked_gun][int(self.picked_stage)][
                                    'offset'][1]
                            except Exception:
                                number, x, y = 0, 0, 0
                        with dpg.group(horizontal=True):
                            self.number_input = dpg.add_input_int(label='次', callback=self.on_number_change,
                                                                  default_value=number, width=self.scaled_width_normal)
                            self.x_input = dpg.add_input_float(label='X', step=0.01, callback=self.on_x_change,
                                                               default_value=x, width=self.scaled_width_normal)
                            self.y_input = dpg.add_input_float(label='Y', step=0.01, callback=self.on_y_change,
                                                               default_value=y, width=self.scaled_width_normal)
                            dpg.add_button(label='删除索引', callback=self.on_delete_stage_click,
                                           width=self.scaled_width_60)
                            dpg.add_button(label='添加索引', callback=self.on_add_stage_click,
                                           width=self.scaled_width_60)
                        dpg.add_separator()
                        with dpg.collapsing_header(label='mouse_re轨迹压枪', default_open=True):
                            with dpg.group(horizontal=True):
                                dpg.add_checkbox(label='启用mouse_re轨迹压枪',
                                                 default_value=self.config.get('recoil', {}).get(
                                                     'use_mouse_re_trajectory', False),
                                                 callback=self.on_use_mouse_re_trajectory_change)
                                dpg.add_input_float(label='回放速度',
                                                    default_value=self.config.get('recoil', {}).get('replay_speed',
                                                                                                    1.0), step=0.1,
                                                    min_value=0.1, max_value=5.0, format='%.2f',
                                                    width=self.scaled_width_normal,
                                                    callback=self.on_mouse_re_replay_speed_change)
                                dpg.add_input_float(label='像素增强比例',
                                                    default_value=self.config.get('recoil', {}).get(
                                                        'pixel_enhancement_ratio', 1.0), step=0.1, min_value=0.1,
                                                    max_value=3.0, format='%.2f', width=self.scaled_width_normal,
                            ...
                            with dpg.group(horizontal=True, tag='mouse_re_combos_group'):
                                pass
                            with dpg.group(horizontal=True):
                                dpg.add_button(label='导入轨迹文件', callback=self.on_import_mouse_re_trajectory_click,
                                               width=self.scaled_width_normal)
                                dpg.add_button(label='清除映射', callback=self.on_clear_mouse_re_mapping_click,
                                               width=self.scaled_width_normal)
                    ...
                    # ================== 5. 配置设置页 ==================
                    with dpg.group(tag='config_settings', show=False):
                        ...
                        with dpg.group(horizontal=True, parent=model_params_group):
                            with dpg.group() as self.dpg_group_tag:
                                self.render_group_combo()
                            dpg.add_button(label='删除组', callback=self.on_delete_group_click,
                                           width=self.scaled_width_60)
                            dpg.add_input_text(label='组名', callback=self.on_group_name_change,
                                               width=self.scaled_width_60)
                        ...
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.infer_model_input = dpg.add_input_text(label='推理模型',
                        ...
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.auto_y_checkbox = dpg.add_checkbox(label='长按左键不锁Y轴',
                                                                    callback=self.on_auto_y_change)
                            self.long_press_duration_input = dpg.add_input_int(label='长按判断阈值', default_value=
                            self.config['groups'][self.group]['long_press_duration'],
                                                                               callback=self.on_long_press_duration_change,
                                                                               width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.target_switch_delay_input = dpg.add_input_int(label='目标转移延迟(ms)',
                                                                               default_value=0, min_value=0,
                                                                               max_value=2000,
                                                                               callback=self.on_target_switch_delay_change,
                                                                               width=self.scaled_width_normal)
                            self.target_reference_class_combo = dpg.add_combo(label='目标参考类别', items=['类别0'],
                                                                              default_value='类别0',
                        ...
                        with dpg.group(horizontal=True, parent=model_params_group):
                            ...
                            dpg.add_button(label='删除键', callback=self.on_delete_key_click,
                                           width=self.scaled_width_60)
                            dpg.add_input_text(label='键名', callback=self.on_key_name_change,
                                               width=self.scaled_width_60)
                            dpg.add_button(label='添加键', callback=self.on_add_key_click, width=self.scaled_width_60)
                        dpg.add_separator(parent=model_params_group)
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.class_priority_input = dpg.add_input_text(label='类别优先级', hint='格式: 0-1-2-3-4',
                                                                           default_value='',
                        ...
                        with dpg.group(horizontal=True, parent=model_params_group):
                            dpg.add_text('类别瞄准配置:')
                            self.class_aim_combo = dpg.add_combo(items=[], label='选择类别',
                                                                 callback=self.on_class_aim_combo_change,
                                                                 width=self.scaled_width_normal, default_value='')
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.confidence_threshold_input = dpg.add_input_float(label='置信阈值', min_value=0.0,
                                                                                  max_value=1.0, step=0.01,
                                                                                  callback=self.on_confidence_threshold_change,
                                                                                  width=self.scaled_width_normal)
                            self.iou_t_input = dpg.add_input_float(label='IOU', min_value=0.0, max_value=1.0, step=0.01,
                                                                   callback=self.on_iou_t_change,
                                                                   width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.aim_bot_position_input = dpg.add_input_float(label='瞄准部位', min_value=0.0,
                                                                              max_value=1.0, step=0.01,
                                                                              callback=self.on_aim_bot_position_change,
                                                                              width=self.scaled_width_normal)
                            self.aim_bot_position2_input = dpg.add_input_float(label='瞄准部位2', min_value=0.0,
                                                                               max_value=1.0, step=0.01,
                                                                               callback=self.on_aim_bot_position2_change,
                                                                               width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.min_position_offset_input = dpg.add_input_int(label='最小偏移',
                                                                               callback=self.on_min_position_offset_change,
                                                                               width=self.scaled_width_normal)
                            self.aim_bot_scope_input = dpg.add_input_int(label='瞄准范围',
                                                                         callback=self.on_aim_bot_scope_change,
                                                                         width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=model_params_group):
                            self.dynamic_scope_enabled_input = dpg.add_checkbox(label='动态范围',
                                                                                callback=self.on_dynamic_scope_enabled_change)
                            self.dynamic_scope_min_scope_input = dpg.add_input_int(label='最小范围', min_value=0,
                                                                                   max_value=2000,
                                                                                   callback=self.on_dynamic_scope_min_scope_change,
                                                                                   width=self.scaled_width_normal)
                            self.dynamic_scope_shrink_ms_input = dpg.add_input_int(label='收缩时长', min_value=0,
                                                                                   max_value=5000,
                                                                                   callback=self.on_dynamic_scope_shrink_ms_change,
                                                                                   width=self.scaled_width_normal)
                            self.dynamic_scope_recover_ms_input = dpg.add_input_int(label='恢复时长', min_value=0,
                                                                                    max_value=5000,
                        ...
                        with dpg.group(horizontal=True, parent=self.pid_params_group):
                            self.pid_kp_x_input = dpg.add_input_float(label='X轴比例', default_value=0.4, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_kp_x_change,
                                                                      width=self.scaled_width_normal)
                            self.pid_ki_x_input = dpg.add_input_float(label='X轴积分', default_value=0.0, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_ki_x_change,
                                                                      width=self.scaled_width_normal)
                            self.pid_kd_x_input = dpg.add_input_float(label='X轴微分', default_value=0.002, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_kd_x_change,
                                                                      width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=self.pid_params_group):
                            self.pid_kp_y_input = dpg.add_input_float(label='Y轴比例', default_value=0.4, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_kp_y_change,
                                                                      width=self.scaled_width_normal)
                            self.pid_ki_y_input = dpg.add_input_float(label='Y轴积分', default_value=0, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_ki_y_change,
                                                                      width=self.scaled_width_normal)
                            self.pid_kd_y_input = dpg.add_input_float(label='Y轴微分', default_value=0.002, step=0.0001,
                                                                      format='%.4f', callback=self.on_pid_kd_y_change,
                                                                      width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=self.pid_params_group):
                            self.pid_integral_limit_x_input = dpg.add_input_float(label='X轴限幅', default_value=0.0,
                                                                                  min_value=0.0, max_value=100.0,
                                                                                  step=0.0001, format='%.4f',
                                                                                  callback=self.on_pid_integral_limit_x_change,
                                                                                  width=self.scaled_width_normal)
                            self.smooth_x_input = dpg.add_input_float(label='X轴平滑', default_value=0, min_value=0.0,
                                                                      max_value=1000.0, step=0.0001, format='%.4f',
                                                                      callback=self.on_smooth_x_change,
                                                                      width=self.scaled_width_normal)
                            self.smooth_algorithm_input = dpg.add_input_float(label='平滑算法', default_value=1.0,
                                                                              min_value=0.1, max_value=10.0,
                                                                              step=0.0001, format='%.4f',
                                                                              callback=self.on_smooth_algorithm_change,
                                                                              width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=self.pid_params_group):
                            self.pid_integral_limit_y_input = dpg.add_input_float(label='Y轴限幅', default_value=0.0,
                                                                                  min_value=0.0, max_value=100.0,
                                                                                  step=0.0001, format='%.4f',
                                                                                  callback=self.on_pid_integral_limit_y_change,
                                                                                  width=self.scaled_width_normal)
                            self.smooth_y_input = dpg.add_input_float(label='Y轴平滑', default_value=0, min_value=0.0,
                                                                      max_value=1000.0, step=0.0001, format='%.4f',
                                                                      callback=self.on_smooth_y_change,
                                                                      width=self.scaled_width_normal)
                            self.smooth_deadzone_input = dpg.add_input_float(label='平滑禁区', default_value=0.0,
                                                                             min_value=0.0, max_value=50.0, step=0.0001,
                                                                             format='%.4f',
                                                                             callback=self.on_smooth_deadzone_change,
                                                                             width=self.scaled_width_normal)
                        with dpg.group(horizontal=True, parent=self.pid_params_group):
                            self.move_deadzone_input = dpg.add_input_float(label='移动死区', default_value=1.0,
                        ...
                        with dpg.group(parent=trigger_setting_tag):
                            with dpg.group(horizontal=True):
                                ...
                                self.continuous_trigger_input = dpg.add_checkbox(label='持续扳机',
                                                                                 callback=self.on_continuous_trigger_change)
                                self.trigger_recoil_input = dpg.add_checkbox(label='扳机压枪',
                                                                             callback=self.on_trigger_recoil_change)
                            with dpg.group(horizontal=True):
                                self.start_delay_input = dpg.add_input_int(label='扳机前摇', min_value=0,
                                                                           max_value=1000,
                                                                           callback=self.on_start_delay_change,
                                                                           width=self.scaled_width_normal)
                                self.press_delay_input = dpg.add_input_int(label='按键时长', min_value=0,
                                                                           max_value=1000,
                                                                           callback=self.on_press_delay_change,
                                                                           width=self.scaled_width_normal)
                            with dpg.group(horizontal=True):
                                self.end_delay_input = dpg.add_input_int(label='扳机冷却', min_value=0, max_value=1000,
                                                                         callback=self.on_end_delay_change,
                                                                         width=self.scaled_width_normal)
                                self.random_delay_input = dpg.add_input_int(label='随机延迟', min_value=0,
                                                                            max_value=1000,
                                                                            callback=self.on_random_delay_change,
                                                                            width=self.scaled_width_normal)
                            with dpg.group(horizontal=True):
                                self.x_trigger_scope_input = dpg.add_input_float(label='X轴范围', min_value=0.0,
                                                                                 max_value=1.0, step=0.01,
                                                                                 callback=self.on_x_trigger_scope_change,
                                                                                 width=self.scaled_width_normal)
                                self.y_trigger_scope_input = dpg.add_input_float(label='Y轴范围', min_value=0.0,
                                                                                 max_value=1.0, step=0.01,
                                                                                 callback=self.on_y_trigger_scope_change,
                                                                                 width=self.scaled_width_normal)
                            with dpg.group(horizontal=True):
                                self.x_trigger_offset_input = dpg.add_input_float(label='X轴偏移', min_value=0.0,
                                                                                  max_value=1.0, step=0.01,
                                                                                  callback=self.on_x_trigger_offset_change,
                                                                                  width=self.scaled_width_normal)
                                self.y_trigger_offset_input = dpg.add_input_float(label='Y轴偏移', min_value=0.0,
                                                                                  max_value=1.0, step=0.01,
                            ...
                            with dpg.drawlist(width=self.scaled_width_small, height=self.scaled_height_normal):
                                ...
                                try:
                                    ...
                                    dpg.draw_rectangle((x_trigger_offset, y_trigger_offset),
                                                       (x_trigger_offset + 50 * x_trigger_scope,
                                                        y_trigger_offset + 100 * y_trigger_scope), fill=(255, 0, 0),
                                                       tag='small_rect')
                                except:
                                    pass
                ...
                # --- 底部按钮区 (保存配置/启动) ---
                with dpg.group(horizontal=True):
                    self.start_button_tag = dpg.add_button(label='启动', callback=self.on_start_button_click,
                                                           width=self.scaled_width_normal)
            ...
            with dpg.group():
                ...
                with dpg.group(horizontal=True):
                    version_text = dpg.add_text(f'版本: {VERSION} {UPDATE_TIME}')
    ...
    # 以下省略了剩余的数百个回调方法（on_*_change），但您可以直接从原 core.py 中复制粘贴
    # 注意：请确保复制时没有调用任何 self.buff_single 相关的方法。
    # 若原代码中有调用 self.verify() 的地方，请将其注释或删除。
    ...
    def on_flash_class_change(self, sender, app_data):
        self.config['auto_flashbang']['flash_class'] = app_data
        print(f"闪光类别已更改为: {app_data}")
    ...
    def update_auto_flashbang_ui_state(self):
        try:
            ...
            auto_flashbang_controls = [
                ...
                'auto_flashbang_keep_time_input',
                'auto_flashbang_search_region_slider',
                'auto_flashbang_threshold_slider'
            ]

            for control_tag in auto_flashbang_controls:
                if dpg.does_item_exist(control_tag):
                    dpg.configure_item(control_tag, enabled=True)
        except Exception as e:
    ...
    def on_select_model_click(self, sender, app_data):
        try:
            ...
            filetypes = [('模型文件', '*.onnx *.engine'), ('所有文件', '*.*')]
    ...
    def update_class_aim_inputs(self):
        ...
        if not isinstance(class_config, dict):
            base_val = float(class_config)
            class_config = {'aim_bot_position': base_val, 'aim_bot_position2': base_val, 'confidence_threshold': 0.5, 'iou_t': 1.0}
            key_cfg['class_aim_positions'][self.current_selected_class] = class_config
    ...
    def on_is_trt_change(self, sender, app_data):
        ...
        if app_data:
            ...
            if current_model.endswith('.engine'):
                ...
                if dopa_path and dopa_path.endswith('.ZTX') and getattr(self, 'decrypted_model_data', None) is not None:
                    self.config['groups'][self.group]['infer_model'] = dopa_path
                    self.refresh_engine()
                    dpg.set_value(self.infer_model_input, dopa_path)
                    return
            elif current_model.endswith('.ZTX') and getattr(self, 'decrypted_model_data', None) is not None:
                self.refresh_engine()
                return
            onnx_path = self.config['groups'][self.group].get('original_infer_model', None)
            if onnx_path and os.path.exists(onnx_path):
        ...
        try:
            ...
            if session is not None:
                ...
                if len(outputs) > 0:
                    ...
                    if len(onnx_shape) >= 2:
                        if onnx_shape[-2] > 4:
                            return onnx_shape[-2] - 4
                        if onnx_shape[-1] > 5:
                            return onnx_shape[-1] - 5
                del session
            return 5
        except Exception as e:
    ...
    def detect_and_handle_flashbang(self, boxes, class_ids, model_width, model_height, scores=None):
        ...
        for i, class_id in enumerate(class_ids):
            # 使用配置中的类别ID进行判定
            if class_id == target_flash_class:
