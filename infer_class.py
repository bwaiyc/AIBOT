...
class OnnxRuntimeDmlEngine:
    ...
    def _initialize_session(self):
        ...
        with self._lock:
            ...
            # [新增] 记录模型要求的输入数据类型 (如 float16 或 float)
    ...
    def infer(self, img_input: np.ndarray) -> List[np.ndarray]:
        ...
        # [核心修复] 根据模型要求自动转换数据精度
        if self._input_type == 'tensor(float16)' and img_input.dtype != np.float16:
            img_input = img_input.astype(np.float16)
        elif self._input_type == 'tensor(float)' and img_input.dtype != np.float32:
